...
Then(/^I recieve a Coaching Activity report with data from "([^"]*)"$/) do |date|
  ...
  @systemCoachings.each do |institution|
    ...
    coaching_activities = institution.coaching_activities.select{ |coaching_activity| (TimeUtils.getStartDate(date) >= coaching_activity.date_start && TimeUtils.getEndDate2(date) <= coaching_activity.date_finish) }
